"""
MountainSort sorting that is artifact-aware (AA); similar to scheme 2, but use training data are only selected from times with low motion noise.
User needs to provide the following:
- Length of the timewindow for determining artifact
- A boolean array of whether each timewindow is artifact polluted or not.
  - Length of the array should be ceil(duration_of_recording / length_of_timewindow)
"""
...
from .SchemeAaSortingParameters import SchemeAaSortingParameters
...
from ..core.get_sampled_recording_for_training import get_selected_sampled_recording_for_training
from ..core.get_times_labels_from_sorting import get_times_labels_from_sorting
from ..core.Timer import Timer


def sorting_scheme_aa(
    recording: si.BaseRecording, *,
    sorting_parameters: SchemeAaSortingParameters,
    ...
    """MountainSort 5 sorting scheme AA

    Args:
        recording (si.BaseRecording): SpikeInterface recording object
        sorting_parameters (SchemeAaSortingParameters): Sorting parameters
    ...
    if sorting_parameters.training_ratio is not None:
        print(f'Using training recording of ratio {sorting_parameters.training_ratio}')
        tt = Timer('SCHEME2 get_sampled_recording_for_training')
        training_recording, _ = get_selected_sampled_recording_for_training(
            recording=recording,
            training_ratio=sorting_parameters.training_ratio,
            max_training_duration_sec=sorting_parameters.max_training_duration_sec,
            window_len_sec=sorting_parameters.artifact_winlen_sec,
            artifact_mask=sorting_parameters.artifact_winmask
        )
        tt.report()
    else:
    ...
    tt = Timer('SCHEMEAA training_recording.get_traces')
    ...
    tt = Timer('SCHEMEAA training classifier step 1')
    ...
    tt = Timer('SCHEMEAA adding snippets from phase 1 sorting')
    ...
    tt = Timer('SCHEMEAA fitting models')
    for m in range(M):
    ...
    if sorting_parameters.classification_chunk_sec is None:
        chunk_size = int(np.ceil(100e6 / recording.get_num_channels())) # size of chunks in samples
    else:
        chunk_size = int(np.ceil(sorting_parameters.classification_chunk_sec * recording.sampling_frequency))
    ...
    for i, chunk in enumerate(chunks):
        ...
        tt = Timer('SCHEMEAA loading traces')
        ...
        tt = Timer('SCHEMEAA detecting spikes')
        ...
        print(f'Scheme AA detected {len(times_chunk)} spikes in chunk {i + 1} of {len(chunks)}')
        tt.report()

        print('Extracting and classifying snippets')
        tt = Timer('SCHEMEAA extracting and classifying snippets')
        ...
        tt = Timer('SCHEMEAA updating events')
    ...
    tt = Timer('SCHEMEAA concatenating results')
    ...
    tt = Timer('SCHEMEAA label mapping')
    if reference_snippet_classifiers is not None:
    ...
    tt = Timer('SCHEMEAA creating sorting object')
